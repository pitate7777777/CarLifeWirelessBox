...
强制使用协议版本 `1.0`，确保与 CarWith 兼容。`VersionDetector` 检测高版本（7.2+）时标记需要协议转换。
...
- ✅ 触摸服务（TouchService，CarLife 触摸事件解析 + 坐标转换）
- ✅ 无障碍服务（CarAccessibilityService，dispatchGesture 触摸注入）
- ✅ 连接服务（ConnectionService 前台服务 + mDNS 广播 + 子服务协调）
- ✅ UI 界面（主界面 / 设置 / 日志查看器 + MediaProjection 授权）

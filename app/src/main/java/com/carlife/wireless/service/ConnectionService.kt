...
class ConnectionService : Service() {
    ...
    private fun startHuRole() {
        ...
        try {
            // 自动检测手机 B 的 IP 地址
            val phoneBIp = detectPhoneBIp()
            LogUtils.i(TAG, "Phone B IP: $phoneBIp (auto-detected)")
            // 保存检测到的 IP 到设置，方便下次使用
            com.carlife.wireless.util.SettingsManager.setPhoneBIp(this, phoneBIp)
     ...
     *
     * 检测策略（按优先级）：
     * 1. USB 网络共享模式：使用 USB 网关 IP（192.168.42.129）
     * 2. WiFi 热点模式：使用 WiFi 网关 IP（热点拥有者的 IP，通常为 192.168.43.1）
     * 3. 手动配置的 IP（Settings 中保存的值）
     * 4. 默认 USB 网关 IP
     */
    private fun detectPhoneBIp(): String {
        // 1. 检查是否通过 USB 网络共享连接
        val usbIp = com.carlife.wireless.util.NetworkUtils.getUsbNetworkIp()
        if (usbIp != null) {
            LogUtils.i(TAG, "检测到 USB 网络: $usbIp, 使用 USB 网关地址")
            return com.carlife.wireless.util.Constants.IpAddress.USB_TETHERING_LOCAL
        }

        // 2. 检查是否通过 WiFi 热点连接
        if (com.carlife.wireless.util.NetworkUtils.isWifiConnected(this)) {
            val gatewayIp = com.carlife.wireless.util.NetworkUtils.getWifiGatewayIp(this)
            if (gatewayIp != null) {
                LogUtils.i(TAG, "检测到 WiFi 热点网关: $gatewayIp")
                return gatewayIp
            }
        }

        // 3. 使用手动配置的 IP
        val savedIp = com.carlife.wireless.util.SettingsManager.getPhoneBIp(this)
        LogUtils.i(TAG, "使用配置的 IP: $savedIp")
        return savedIp
    }

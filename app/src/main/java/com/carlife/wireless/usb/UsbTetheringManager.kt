...
class UsbTetheringManager(private val context: Context) {
    ...
    /**
     * 检测是否有活跃的 USB 网络接口（usb0 / rndis0 / ncm0）
     *
     * 关键修复：不仅检查接口名称，还检查接口是否 UP 且有 IPv4 地址。
     * 旧逻辑仅检查名称，导致在没有 USB 连接时也误报为已连接
     * （因为某些设备的 usb0/rndis0 接口在内核中始终存在，只是 DOWN 状态）。
     */
    private fun hasUsbNetworkInterface(): Boolean {
        return try {
            val interfaces = NetworkInterface.getNetworkInterfaces()
            interfaces?.toList()?.any { intf ->
                isUsbInterface(intf) && intf.isUp &&
                    intf.inetAddresses.toList().any { it is Inet4Address && !it.isLoopbackAddress }
            } ?: false
        } catch (e: Exception) {
     ...
     * 判断是否为 USB 相关网络接口
     */
    private fun isUsbInterface(intf: NetworkInterface): Boolean {
        val name = intf.name.lowercase()
        return name.startsWith("usb") || name.startsWith("rndis") || name.startsWith("ncm")
    }

    /**
     * 获取 USB 网络接口的本机 IP
     * 仅返回 UP 状态下的有效 IPv4 地址
     */
    fun getUsbInterfaceIp(): String? {
        return try {
            val interfaces = NetworkInterface.getNetworkInterfaces()
            interfaces?.toList()?.forEach { intf ->
                if (isUsbInterface(intf) && intf.isUp) {
    ...
    fun getUsbNetworkInfo(): String {
        ...
        try {
            val interfaces = NetworkInterface.getNetworkInterfaces()
            interfaces?.toList()?.forEach { intf ->
                if (isUsbInterface(intf)) {
                    sb.appendLine("网口: ${intf.name}, UP=${intf.isUp}, hasIPv4=${
                        intf.inetAddresses.toList().any { it is Inet4Address && !it.isLoopbackAddress }
                    }")
                    intf.inetAddresses.toList().forEach { addr ->

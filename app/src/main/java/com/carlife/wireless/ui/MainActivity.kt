...
class MainActivity : AppCompatActivity() {
    ...
    private fun updateIPAddress() {
        val (mode, ip) = com.carlife.wireless.util.NetworkUtils.getConnectionModeAndIp(this)
        val modeLabel = when (mode) {
            "wifi" -> "WiFi"
            "usb" -> "USB"
            else -> "--"
        }
        binding.tvIpAddress.text = "IP: ${ip ?: "--"} ($modeLabel)"

        // 自动检测并显示手机 B 的 IP（WiFi 热点网关）
        if (mode == "wifi") {
            val gatewayIp = com.carlife.wireless.util.NetworkUtils.getWifiGatewayIp(this)
            if (gatewayIp != null) {
                binding.tvCarIp.text = "手机 B: $gatewayIp"
